...
class StartMediumUser:
    def __init__(self):
        self.board = {(x, y): " " for y in range(3, 0, -1)
                      for x in range(1, 4)}
        self.available_co = list(self.board.keys())
        self.moves = {"X": 0, "O": 0, " ": 0}
        self.user = User1("user")
        self.comp = Computer1("comp")
        self.game_on = True
        self.whos_turn = self.user  # can change in future
        self.winner = ""
        self.medium_user()

    def medium_user(self):
        while self.game_on:
            StartMediumUser.draw_board(self.board)
            if self.whos_turn.p_name == "comp":
                move, m_char = self.user.check_commit(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
...
class StartUserMedium:
    def __init__(self):
        self.board = {(x, y): " " for y in range(3, 0, -1)
                      for x in range(1, 4)}
        self.available_co = list(self.board.keys())
        self.moves = {"X": 0, "O": 0, " ": 0}
        self.user = User2("user")
        self.comp = Computer3("comp")
        self.game_on = True
        self.whos_turn = self.user  # can change in future
        self.winner = ""
        self.user_medium()
    ...
    def user_medium(self):
        while self.game_on:
            StartUserMedium.draw_board(self.board)
            if self.whos_turn.p_name == "user":
                move, m_char = self.user.check_commit2(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.comp
            else:
                move, m_char = self.comp.make_move2(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.user
...
class Computer3(Player):
    def __init__(self, name, level="Medium", p_char="O"):
        super().__init__(name, p_char)
        self.play_char = p_char
        self.level = level
...
class Computer1(Player):
    def __init__(self, name, level="Medium", p_char="X"):
        super().__init__(name, p_char)
        self.play_char = p_char
        self.level = level
...
class StartMediumMedium:
    def __init__(self):
        self.board = {(x, y): " " for y in range(3, 0, -1)
                      for x in range(1, 4)}
        self.available_co = list(self.board.keys())
        self.moves = {"X": 0, "O": 0, " ": 0}
        self.medium1 = Computer1("medium1")  # X
        self.medium2 = Computer3("medium2")  # O
        self.game_on = True
        self.whos_turn = self.medium1  # can change in future
        self.winner = ""
        self.medium_medium()

    def medium_medium(self):
        while self.game_on:
            StartMediumMedium.draw_board(self.board)
            if self.whos_turn.p_name == "medium1":
                move, m_char = self.medium1.make_move(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.medium2
            else:
                move, m_char = self.medium2.make_move2(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.medium1
            self.winner = self.declare_winner()
            if self.winner != "":
                self.game_on = False
                self.draw_board(self.board)
...
class StartMediumEasy:
    def __init__(self):
        ...
        self.moves = {"X": 0, "O": 0, " ": 0}
        self.medium1 = Computer1("medium1")  # X
        self.easy2 = Computer2("easy2")  # O
        self.game_on = True
        self.whos_turn = self.medium1  # can change in future
        self.winner = ""
        self.medium_easy()

    def medium_easy(self):
        while self.game_on:
            StartMediumEasy.draw_board(self.board)
            if self.whos_turn.p_name == "medium1":
                move, m_char = self.medium1.make_move(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.easy2
            else:
                move, m_char = self.easy2.make_move2(self.available_co)
                self.available_co.remove(move)
                self.board[move] = m_char
                self.moves[m_char] += 1
                self.whos_turn = self.medium1
...
def final_game():
    while True:
        ...
        if start == 'start user easy' or start == 'start easy user':
            ...
            StartEasyEasy()
        elif start == 'start medium user':
            StartMediumUser()
        elif start == 'start user medium':
            StartUserMedium()
        elif start == 'start medium medium':
            StartMediumMedium()
        elif start == 'start medium easy' or start == 'start easy medium':
            StartMediumEasy()
        elif start == 'exit':
            sys.exit()
        else:
